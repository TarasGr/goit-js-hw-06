...
// First attempt ;)
// const getUsersWithGender = (users, gender) => users.reduce((acc, item) => { 
//     if (item.gender === gender) acc = [...acc, item.name];
//     return acc;
// }, []);
// Second attempt, more declarative
// We also can rechange .reduce () method on the .map(user => user.name), I choose .reduce for practice to use
const getUsersWithGender = (users, gender) => users.filter(user => user.gender === gender).reduce((acc, item) => acc = [...acc, item.name], []);
...
const getUsersWithAge = (users, min, max) => users.filter(user => user.age >= min && user.age < max); // user.age <= max как определить условие "до ..."?
...
// In this task I choose .map method instead .reduce method 
...
/*Задание 10: Получить массив всех умений всех пользователей (поле skills), при этом не должно быть повторяющихся умений и 
они должны быть отсортированы в алфавитном порядке. */
const getSortedUniqueSkills = users => users.reduce((acc, user) => acc = [...acc, ...user.skills], []).reduce((acc, skill) => {
